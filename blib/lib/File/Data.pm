...
# $Id: File::Data.pm,v 0.01 2000/03/22 14:15:36 richard Exp $
#
...
replace.  With a failure, normally the only message is 'permission denied', 
...
sub do {
	...
	$self->_enter('do');
	$self->_debug('in: '.Dumper($self)) if $File::Data::DEBUG;

	print $self->$call(@_);

	$self->_debug('out: '.Dumper($self)) if $File::Data::DEBUG;
	$self->_leave('do');
...
perl -MFile::Data -e "File->Data->new('./test.txt')->insert('123', '456')";
